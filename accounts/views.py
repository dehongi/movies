...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["user"] = user
        # Assuming there are related models for notes/media, but since not specified, we'll use placeholders
        # context['recent_notes'] = user.notes.order_by('-created_at')[:5]  # If notes exist
        # context['recent_media'] = user.media.order_by('-uploaded_at')[:5]  # If media exist
        return context


class PlanView(LoginRequiredMixin, DetailView):
    model = CustomUser
    template_name = "accounts/plan.html"
    context_object_name = "user"
